...
class Inductor(Impedance):
    def __init__(self, Z0, ZL, L, f0, domain="impedance"):
        self.Z0 = Z0;
        self.ZL = ZL;
        self.L = L;
        self.f0 = f0;
    ...
    def addToSmithChart(self,theta,cadena="Inductor",color="#ff9955ff"):
        self.cadena = cadena;
        alpha = np.linspace(0.0, 1.0, 1000);
        if self.domain == "impedance":
            Zpath = (self.Zin + 1j * alpha *2 * np.pi * self.f0 * self.L);
            gamma = (Zpath - self.Z0) / (Zpath + self.Z0);

        else:
            Ypath = (self.Yin - 1j * alpha /(2 * np.pi * self.f0 * self.L));
